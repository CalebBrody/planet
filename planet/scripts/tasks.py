...
def gym_cheetah(config, params):
  ...
  env_ctor = functools.partial(
      _gym_env, action_repeat, max_length, 'HalfCheetah-v3')
...
def gym_racecar(config, params):
  ...
  env_ctor = functools.partial(
      _gym_env, action_repeat, max_length, 'CarRacing-v0',
      obs_is_image=True)
...
def _gym_env(action_repeat, max_length, name, obs_is_image=False):
  import gym
  env = gym.make(name)
  env = control.wrappers.ActionRepeat(env, action_repeat)
  env = control.wrappers.NormalizeActions(env)
  env = control.wrappers.LimitDuration(env, max_length, ignore_done=True)
  if obs_is_image:
